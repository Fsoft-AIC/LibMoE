...
def e_step(X, Y, beta0, beta1, a, b, sigma, K, n_jobs=None):
    """
    E-step: responsibilities r[n,i] using parallel processing.
    """
    n = len(X)
    k = len(beta0)
    r = np.zeros((n, k))
    ...
    if n_jobs is None:
        n_jobs = min(cpu_count(), 8)  # Use at most 8 cores
...
def m_step_gating(X, Y, r, beta0, beta1, a, b, sigma, K, lr=5e-3, steps=10, n_jobs=None):
    """
    Gating parameter update via gradient-based approach with parallel processing.
    """
    k = len(beta0)
    n = len(X)
    b0_new = beta0.copy()
...
def fit_topk_moe(X, Y, k, K, true_params=None, max_iter=200, tol=1e-6, lr=5e-3, steps=10, n_jobs=None):
    """
    Fit a top-K gating MoE via an EM-like approach with parallel processing.
    """
    if n_jobs is None:
        n_jobs = min(cpu_count(), 8)  # Use at most 8 cores
    ...
    # For small datasets, don't use multiprocessing for log-likelihood
    use_mp_for_ll = n >= 1000 and n_jobs > 1
    ...
    for it in range(max_iter):
        # E-step
        r = e_step(X, Y, beta0, beta1, a, b, sigma, K, n_jobs=n_jobs)

        # M-step
        a, b, sigma = m_step_experts(X, Y, r, a, b, sigma)
        beta0, beta1 = m_step_gating(X, Y, r, beta0, beta1, a, b, sigma, K, lr=lr, steps=steps, n_jobs=n_jobs)
...
def run_single_trial(args):
    ...
    beta0_hat, beta1_hat, a_hat, b_hat, sigma_hat = fit_topk_moe(
        ...
        n_jobs=1  # Force single process inside the worker
    )
...
if __name__ == "__main__":
    ...
    n_processes = min(cpu_count(), 64)  # Use at most 8 cores
    ...
    for ax_idx, (title, k_fit, K_fit) in enumerate(scenarios):
        ...
        for n in tqdm(sample_sizes):
            ...
            with Pool(processes=n_processes) as pool:
                errors = pool.map(run_single_trial, trial_args)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig("voronoi_style_error2.pdf")
    print("Experiment completed. Results saved to voronoi_style_error.pdf")
