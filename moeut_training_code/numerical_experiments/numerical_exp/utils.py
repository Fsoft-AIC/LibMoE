...
def plot_expert_predictions(model, X, Y, filename='expert_predictions.pdf'):
    ...
    for j in range(model.num_shared):
        mean = model.a_shared[j] * X_grid + model.b_shared[j]
        plt.plot(X_grid, mean, label=f'Shared Expert {j+1}')
...
def evaluate_model(model, X, Y):
    ...
    expert_means = np.zeros((len(X), model.num_experts + model.num_shared))
    for j in range(model.num_experts):
        expert_means[:, j] = 1/2 * (model.a[j] * X + model.b[j])
    for j in range(model.num_shared):
        expert_means[:, model.num_experts + j] = 1/2 * (model.a_shared[j] * X + model.b_shared[j])

    # Compute model predictions
    predictions = np.sum(gating_probs * expert_means[:, :model.num_experts], axis=1) + np.sum(gating_probs * expert_means[:, model.num_experts:], axis=1)
