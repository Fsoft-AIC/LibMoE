...
def run_experiment(config: Optional[ExperimentConfig] = None, k_true: int = 3, k_fit: int = 3, K: int = 2, num_shared: int = 1, router_type: str = "softmax", max_iter: int = 100, seed: int = 42, sample_size: int = 1000) -> Dict[str, Any]:
    ...
    if config is not None:
        ...
        omega_shared_true = config.omega_shared_true
    else:
        ...
        omega_shared_true = np.random.randn(num_shared)

    # Generate data
    # Use init_params for parameter initialization
    X, Y = generate_data(sample_size, beta0_true, beta1_true, a_true, b_true, sigma_true, a_shared_true, b_shared_true, sigma_shared_true, omega_shared_true, K,  router_type)
    ...
    if num_shared > 0:
        ...
        print(f"omega_shared: {omega_shared_true}")
    ...
    # add shared parameters if any
    if num_shared > 0:
        print(f"a_shared: {model.a_shared}")
        print(f"b_shared: {model.b_shared}")
        print(f"sigma_shared: {model.sigma_shared}")
        print(f"omega_shared: {model.omega_shared}")
