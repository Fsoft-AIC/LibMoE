...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    def __init__(self, dmodel: int, n_experts: int, expert_size: int, n_heads: int,
        ...
        assert self.perplexity_reg_mode in {"global", "time", "step", "layers_time", "deepseek", "standard"}
        ...
        # init self.values and self.values_shared
    ...
    def add_perplexity_reg_standard(self, sel_aux: torch.Tensor, sel_index: torch.Tensor, bsz: int, seq_len: int):
        ...
        if self.perplexity_reg > 0:
            loss = lambda: self.perplexity_reg * (aux_loss * sel_aux.mean(dim = 1)).sum(dim = 1).mean()
    ...
    def deepseek_topk(self, scores: torch.Tensor, k: int, bsz: int, seq_len: int):
        ...
        score_mask = (
            ...
            .expand(
                bsz * seq_len, self.n_group, self.n_experts // self.n_group
            )
            .reshape(bsz * seq_len, -1)
        )  # [n, e]
        tmp_scores = scores_for_choice.masked_fill(~score_mask.bool(), 0.0)  # [n, e]
        _, topk_idx = torch.topk(
            tmp_scores, k=self.n_heads, dim=-1, sorted=False
        )
        topk_idx = topk_idx.view(bsz, seq_len, -1)
        topk_weight = scores.gather(1, topk_idx)

        # update self.e_score_correction_bias
        counts_expert_usage = torch.bincount(topk_idx.flatten(), minlength=self.n_experts)
        self.e_score_correction_bias.data = self.e_score_correction_bias.data + \
            self.e_score_correction_bias_scale * torch.sign(counts_expert_usage.float().mean() - counts_expert_usage.float())

        return topk_weight, topk_idx


    def deepseek_topk_no_gatingbias(self, scores: torch.Tensor, k: int, bsz: int, seq_len: int):
        # Auxiliary-Loss-Free Load Balancing
    ...
    def deepseek_topk_nogroup(self, scores: torch.Tensor, k: int, bsz: int, seq_len: int):
        scores_for_choice = scores + self.e_score_correction_bias.unsqueeze(0).unsqueeze(0)
        _, topk_idx = torch.topk(scores_for_choice, k=self.n_heads, dim=-1, sorted=False)
        topk_weight = scores.gather(1, topk_idx)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.perplexity_reg_mode in {"deepseek"}:
            ...
            sel_val, sel_index = self.deepseek_topk_no_gatingbias(sel, self.n_heads, bsz, seq_len)
        else:
        ...
        sel_index_shared = torch.zeros(sel_index.size(0), sel_index.size(1), 1, device=sel_index.device)
        sel_val_shared = torch.ones(sel_index.size(0), sel_index.size(1), 1, device=sel_index.device)

        sel_indices_shared = cvmm_prepare_sel2(sel_index_shared.int())
        scores_shared = self.compute_scores(in3, sel_indices_shared, is_shared=True)
