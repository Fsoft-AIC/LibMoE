...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    def __init__(self, dmodel: int, n_experts: int, expert_size: int, n_heads: int,
        ...
        ### ===== DEFINE FOR PERTURBED COSINE ROUTER ===== ###
        self.reduction_dim = int(self.n_experts / 2)

        # expert embedding
        expert_embeddings = torch.empty(self.n_experts, self.reduction_dim)
    ...
    ### ===== DEFINE FOR PERTURBED COSINE ROUTER ===== ###
    def _perturbed_cosine(self, mat1, mat2, eps=1e-4):
        assert mat1.dim() == 2
        assert mat2.dim() == 2
        mat1 = mat1.float() / (torch.norm(mat1.float(), p=2.0, dim=1, keepdim=True).clamp(min=eps) + self.theta)
        # mat2 = mat2.float() / (torch.norm(mat2.float(), p=2.0, dim=1, keepdim=True).clamp(min=eps) + theta)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        ### ===== DEFINE FOR PERTURBED COSINE ROUTER ===== ###
        reduced_inp = self.inp_reduction(sel_input)
        reduced_inp = reduced_inp.reshape(-1, self.reduction_dim)
        with torch.no_grad():
        ...
        sel = self._perturbed_cosine(reduced_inp, self.expert_embeddings)
        sel = self._make_finite(sel)
        sel = sel.view(in1.size(0), in1.size(1), sel.size(-1))
        sel = F.softmax(sel / self.temperature, dim=-1)
        ### ===== =============================== ===== ###
