...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    def __init__(self, dmodel: int, n_experts: int, expert_size: int, n_heads: int,
        ...
        assert self.perplexity_reg_mode in {"global", "time", "step", "layers_time", "standard", "jain_fairness"}
        ...
        # ============== Jain fairness ==============
        self.jain_fairness_loss_scale = 0.1
    ...
    def add_jain_fairness_reg(self, sel_aux: torch.Tensor, sel_index: torch.Tensor, bsz: int, seq_len: int):
        ...
        # calculate the jain fairness loss
        counts_expert_usage = torch.bincount(sel_index.flatten(), minlength=self.n_experts)
        sum_sq = counts_expert_usage.sum().pow(2)
        sq_sum = (counts_expert_usage ** 2).sum()
        jfi = sum_sq / (self.n_experts * sq_sum)
        jain_fairness_loss = 1 - jfi

        if self.perplexity_reg > 0:
            loss = lambda: 0.0001 * (aux_loss * sel_aux_scaled.mean(dim = 1)).sum(dim = 1).mean() + \
                self.jain_fairness_loss_scale * jain_fairness_loss
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if reg_sel is not None:
            ...
            elif self.perplexity_reg_mode in {"jain_fairness"}:
                self.add_jain_fairness_reg(sel_aux, sel_index, bsz, seq_len)
