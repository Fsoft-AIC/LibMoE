...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    def __init__(self, dmodel: int, n_experts: int, expert_size: int, n_heads: int,
        ...
        ### ==== FiLM Layer ====
        self.film_size = self.expert_size // 2
        ...
        # self.get_initializer()(self.values_film, std=(self.film_size) ** -0.5 * weight_scale)
    ...
    def compute_scores(self, input: torch.Tensor, index: CVMMSel, is_film: bool=False) -> Tuple[torch.Tensor, torch.Tensor]:
        if not is_film:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        ### ==== FILM Layer ====
        sel_index_film = torch.zeros(sel_index.size(0), sel_index.size(1), 1, device=sel_index.device)
        sel_val_film = torch.ones(sel_val.size(0), sel_val.size(1), 1, device=sel_val.device)

        sel_indices_film = cvmm_prepare_sel2(sel_index_film.int())
        scores_film = self.compute_scores(in3, sel_indices_film, is_film=True)

        sel_indices_film = sel_indices_film.clone()
        sel_indices_film.reduction_weight = sel_val_film
        sel_indices_film.sel_index = sel_indices_film.out_index
        sel_indices_film.out_index = None

        out_film = cvmm(scores_film, sel_indices_film, self.values_film) + self.values_film_bias
        gamma, beta = out_film.chunk(2, dim=-1)
