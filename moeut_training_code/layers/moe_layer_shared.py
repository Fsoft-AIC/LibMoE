...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # ===== Modified Section for Shared Expert =====
        # Instead of applying dropout and topk to all experts, we:
        # 1. Apply expert dropout only on the non-shared experts (first n_experts-1).
        # 2. Select the top (n_heads - 1) experts from these non-shared experts.
        # 3. Always include the shared expert (the last expert) with its original score.
        non_shared_sel = sel[..., :self.n_experts - 1]
        if self.training and self.expert_dropout > 0:
            mask = torch.rand_like(non_shared_sel) < self.expert_dropout
            non_shared_sel2 = non_shared_sel.masked_fill(mask, inv_val)
        else:
            non_shared_sel2 = non_shared_sel

        non_shared_val, non_shared_index = self.topk(non_shared_sel2, self.n_heads - 1)
        ### norm gate to sum 1
        denominator = non_shared_val.sum(dim=-1, keepdim=True) + 1e-20
        non_shared_val = non_shared_val / denominator

        # shared_val = non_shared_val.sum(-1).unsqueeze(-1)
        shared_val = torch.full(size=sel[..., self.n_experts - 1].unsqueeze(-1).size(), fill_value=1.,
                                dtype=non_shared_index.dtype, device=non_shared_index.device)
        shared_index = torch.full(shared_val.shape, self.n_experts - 1, dtype=non_shared_index.dtype, device=non_shared_index.device)

        sel_val = torch.cat([non_shared_val, shared_val], dim=-1)
        sel_index = torch.cat([non_shared_index, shared_index], dim=-1)
