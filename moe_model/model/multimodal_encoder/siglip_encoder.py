        ...
        if type(images) is list:
            ...
            if self.clip_smoe:
                ...
                for i, (x, num_split) in enumerate(zip(image_pyramids, num_splits)):
                    if i == 0:
                        out_x, auxilarity_loss, vision_id_experts, balance_loss = self.vision_model( pixel_values = x,   return_id_experts = return_id_experts)
                    else:
                        out_x, auxilarity_loss, _, balance_loss = self.vision_model( pixel_values = x,   return_id_experts = return_id_experts)
                ...
                return image_features, torch.stack(auxilarity_losses).mean(), vision_id_experts, torch.stack(balance_losses).mean()
            else:
                image_features = []
                for i, (x, num_split) in enumerate(zip(image_pyramids, num_splits)):
