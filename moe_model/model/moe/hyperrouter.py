...
class HyperRouter(MoeLayer):
    def __init__(self, in_embed_dim=768, out_embed_dim=768, num_of_experts=4, num_selected=2, expert=None, args=None):
        ...
        print("Starting with hyper router")
        # define HyperNetwork 
        ...
        self.recreate_gate_check = False
    ...
    def recreate_gate(self):
        print("""Recontruct weight of the gate""")
        self.hypernet_outputs = self.hypernet(self.hyper_embedding)[0]
        weights_splice = self.hypernet_outputs.reshape([self.num_of_experts, -1])
        self.gate.weight.data = weights_splice[:, :-1]
        self.gate.bias.data = weights_splice[:, -1] 
    ...
    def forward(self, x, return_id_experts = False, is_vision=False): 
        ...
        if x.requires_grad: # check if it is training phase
            ...
            if self.recreate_gate_check == False:
                self.recreate_gate()
                self.recreate_gate_check = True
        ...
        gate_softmax = F.softmax(gate_logits, dim=-1, dtype=torch.float).to(x.device)
        ...
        if x.requires_grad: 
            auxiliary_loss, balance_loss = self.combine_loss(selected_experts=selected_experts, gate_logits=gate_logits, gate_softmax=gate_softmax)
        
        if return_id_experts:
            return output, auxiliary_loss, gate_softmax, balance_loss
        else: 
