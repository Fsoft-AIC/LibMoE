...
@register_moe("dynamic_moe_v4")
class DynamicMoEv4(MoeLayer):
    ...
    def __init__(self, in_embed_dim=768, out_embed_dim=768, num_of_experts=4, num_selected=2, expert=None, args=None):
        ...
        self.aux_loss['kl_loss'] = self.kl_divergence_loss
        ...
        if self.current_steps > 0 and percent_progress % 10 == 0 and percent_progress != self.last_percent:
            ...
            if is_log:
                with open(f"/cm/shared/anonymous_H102/toolkitmoe/logs/{self.name_logs}", 'a') as file:
                    ...
                    print(f"{timestamp}: Apply Dynamic Expert at {percent_progress}% data: {self.dynamic_activate}\n")
    ...
    def kl_divergence_loss(self, gate_logits, gate_softmax):
        num_classes = gate_softmax.size(-1)
        uniform_dist = torch.full_like(gate_softmax, 1.0 / num_classes)
        max_value, _ = torch.max(gate_softmax, dim=-1)
        
        # Tính KL Divergence
        kl_divergence = torch.sum(gate_softmax * torch.log((gate_softmax + 1e-12) / (uniform_dist + 1e-12)), dim=-1)
        
        # Nhân với (1 - max_value) và lấy trung bình để trả về một scalar
        loss = (1 - max_value) * (1 / (kl_divergence + 1e-12))
        
        return loss.mean()  # Trả về trung bình của toàn bộ batch
    ...
    def combine_loss(self, selected_experts, gate_softmax, gate_logits):
        ...
        print(f"balance_loss: { balance_loss * self.args.balance_loss_coef }, Entropy: {entropy_loss * self.args.router_z_loss_coef}")
    ...
    def strategy_mix_loss(self, selected_experts, gate_softmax, gate_logits):
        if self.dynamic_activate == False:
            ...
            # compute router_z_loss
            loss = self.args.router_z_loss_coef * self.kl_divergence_loss(gate_logits = gate_logits, gate_softmax = gate_softmax)
