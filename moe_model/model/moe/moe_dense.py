...
@register_moe("competesmoe_moe_dense")
class moe_dense(MoeLayer):
    ...
    def set_total_steps(self, total_steps, id_layer, prob_flips_final):
                        ...
                        if not found:
                            for j in range(i + 1, flip_steps):
                                if (freq_updated[j] < self.args.max_compete_in_iter) and (not candidate_current[j]):
                                    ...
                                    found = True
                                    break
            with open("/cm/shared/anonymous_H102/toolkitmoe/moe_model/model/moe/file_path.txt", "a") as f:
        ...
        file_path = '/cm/shared/anonymous_H102/toolkitmoe/moe_model/model/moe/prob_flips_final.pt'

        # Save the dictionary to a file using torch.save
        torch.save(prob_flips_final, file_path)
    ...
    def topk_expert(self, gate_logits, active_all = False):
        """
        ...
        if active_all:
            num_selected = self.num_of_experts
        weights, selected_experts = torch.topk(gate_softmax, num_selected)
        return weights, selected_experts, gate_softmax
    def router_policy(self, x, active_comp = False):
        """
        ...
        weights, selected_experts, gate_softmax = self.topk_expert(gate_logits=gate_logits, active_all=active_comp)
    ...
    def combine_loss(self, selected_experts, gate_softmax, gate_logits, acitve_zloss =True, active_all = False):
        if active_all:
            balance_loss = torch.tensor(0.0, device=selected_experts.device, dtype=selected_experts.dtype)
        else:
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        """
        active_comp = x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1
        output = torch.zeros(x.shape[0], x.shape[1], self.out_embed_dim, device=x.device, dtype=x.dtype)

        # Normal gating for expert selection
        gate_weights, gate_selected_experts, gate_softmax, gate_logits = self.router_policy(x, active_comp)
        ...
        if active_comp:
            ...
            output, div_loss = self.compute_moe(
                weights=gate_weights,
                selected_experts=gate_selected_experts,
                results=output,
                x=x, 
                return_topk_outputs=active_comp
            )
        else:
        ...
        if x.requires_grad or return_id_experts: 
            ...
            if active_comp:
                ...
                # infor_aux['div_loss'] = div_loss.clone().detach(),
            # selected_experts = gate_selected_experts
