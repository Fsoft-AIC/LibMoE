...
class CompeteSMoE(MoeLayer):
    def __init__(self, in_embed_dim=768, out_embed_dim=768, num_of_experts=4, num_selected=2, expert=None, args=None):
        ...
        # self.register_buffer('prob_flips', torch.zeros(19752))
        print(f"Init gate weight follow: {self.args.std_gate}")
        self.init_gate_weights(self.args.std_gate)
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        active = torch.tensor(0.0, device=x.device, dtype=x.dtype)
        infor_aux = {}
        ...
        if x.requires_grad and self.current_steps >= self.step_warm and self.prob_flips[self.current_steps - self.step_warm].item() == 1:
            ...
            infor_aux = {
                ...
                "active": active.clone().detach(),
            }
            
        else:
            if x.requires_grad and self.current_steps >= self.step_warm:
                active = torch.tensor(1.0, device=x.device, dtype=x.dtype)
