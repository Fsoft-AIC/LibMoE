...
class CompeteSMoEv6(MoeLayer):
    ...
    def compute_moe_infer(self, selected_experts, weights, results, x):
        ...
        # Tìm 2 giá trị lớn nhất trên mỗi hàng (top-2 expert)
        # top2_values, _ = torch.topk(luna_gating_softmax, 2, dim=1)
        # top2_values =weights
        # Tính Balance Ratio (BR)
        # balance_ratio = 1 - torch.abs(top2_values[:, 0] - top2_values[:, 1])

        # Tính Entropy-Based Balance Score (H)
        # entropy_score = - (top2_values[:, 0] * torch.log2(top2_values[:, 0] + 1e-9) + 
        #                 top2_values[:, 1] * torch.log2(top2_values[:, 1] + 1e-9))

        # Tính trung bình trên tất cả các token
        # mean_balance_ratio = balance_ratio.mean().item()
        # mean_entropy_score = entropy_score.mean().item()

        # Tạo DataFrame chứa kết quả trung bình
        # df_mean_results = pd.DataFrame({
        #     "Mean Balance Ratio (BR)": [mean_balance_ratio],
        #     "Mean Entropy Score (H)": [mean_entropy_score]
        # })

        # # # Lưu vào file JSON
        # json_filename = "/cm/shared/anonymous_H102/toolkitmoe/moe_model/model/moe/gate_moe_balance_metrics_v27.json"
        # df_mean_results.to_json(json_filename, orient="records", indent=4)
