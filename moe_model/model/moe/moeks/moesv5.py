...
import pandas as pd
import copy 
@register_moe("moeskv5")
class MoESKv5(MoeLayer):
    def __init__(self, in_embed_dim=768, out_embed_dim=768, num_of_experts=4, num_selected=2, expert=None, args=None):
        ...
        self.register_buffer("global_balance", torch.zeros(self.num_of_experts, dtype=torch.float32, requires_grad=False))
    ...
    def router_policy(self, x):
        ...
        with torch.no_grad():
            ...
            self.global_balance = self.global_balance.to(x.device).to(x.dtype)
            flat_weights  = weights.reshape(-1).to(x.device).to(x.dtype)
            flat_experts  = selected_experts.reshape(-1).to(x.device)

            # balance loss of history and currently
            alpha = 0.8          
            self.global_balance.mul_(alpha)                      
            self.global_balance.index_add_(0, flat_experts,
                                (1 - alpha) * flat_weights)
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        auxiliary_loss, balance_loss, router_z_loss = self.combine_loss(gate_selected_experts, gate_softmax, gate_logits)
        # auxiliary_loss += bal_loss*0.0001
        ...
        infor_aux = {
            ...
            "bal_global": bal_loss.clone().detach()
        }
