...
import pandas as pd
import copy 
@register_moe("moeksv2")
class MoEKSv2(MoeLayer):
    def __init__(self, in_embed_dim=768, out_embed_dim=768, num_of_experts=4, num_selected=2, expert=None, args=None):
        ...
        self.gate_top = nn.Linear(num_of_experts - 1, self.num_selected, bias=True)
        ...
        # self.init_gate_top_weights()
        self.init_expert_weights()
        # if self.args.sparse_upcycling == False:
        self.init_shared()
    def init_gate_top_weights(self, std = 0.02):
        """
        ...
        if init_weight == False:
            ...
            return 
        device = self.gate_top.weight.device if self.gate_top.weight.device != torch.device('meta') else torch.device('cpu')
        # device = self.gate_top.weight.device
        gate_generator = torch.Generator(device=device)
        gate_generator.manual_seed(42)
        """
        Initialize the weights and bias of the gating layer.
        """
        nn.init.normal_(self.gate_top.weight, mean=0.0, std=std, generator=gate_generator)
        if self.gate_top.bias is not None:
            nn.init.constant_(self.gate_top.bias, 0.0)
    ...
    def shared_knowlegde(self, x, gate_softmax, output_shared):
        ...
        for i in range(self.num_of_experts):
            ...
            if share_loss == None:
                ...
                share_loss += F.mse_loss(out_exp, output_shared[batch_idx, token_idx])
        share_loss = share_loss / (self.num_of_experts - self.num_selected)
    ...
    def router_policy(self, x):
        ...
        weights_logit = self.gate_top(gate_logits)
        weights_top = torch.sigmoid(weights_logit)
        # Select experts using top-k gating
        _, selected_experts, gate_softmax = self.topk_expert(gate_logits=gate_logits)
        
        weights = weights_top / torch.sum(weights_top, dim=-1, keepdim=True).to(x.dtype)

        return weights, selected_experts, gate_softmax, weights_logit
    ...
    def forward(self, x, return_id_experts=False, is_vision=False):
        ...
        if x.requires_grad and (self.current_steps < self.stop_share) and (self.current_steps + 1) % (self.nb_wakers + self.id_layer) == 0:
            ...
            auxiliary_loss += shared_loss*0.0001
