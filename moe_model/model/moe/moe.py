...
class MoeLayer(nn.Module):
    ...
    def topk_expert(self, gate_logits):
        ...
        """
        gate_softmax = F.softmax(gate_logits, dim=-1, dtype=torch.float).to(gate_logits.dtype)
    ...
    def compute_moe(self, selected_experts, weights, results, x):
        ...
        for i, expert in enumerate(self.experts):
            ...
            results[batch_idx, token_idx] += weights[batch_idx, token_idx, topk_idx].unsqueeze(0).T * expert(x[batch_idx, token_idx])
    ...
    def combine_loss(self, selected_experts, gate_softmax, gate_logits):
        ...
        router_z_loss = self.zloss(gate_logits)
    ...
    def forward(self, x, return_id_experts = False):
        ...
        # compute loss
        auxiliary_loss = self.combine_loss(selected_experts, gate_softmax, gate_logits)
