        ...
        **kwargs
        ...
        if self.config.training:
            if self.config.mlp_smoe or self.config.clip_smoe:
                ...
                out["trainer_logs"]['balance_loss_clip'] = balance_losses_clip.sum(dim=-1).mean().detach().clone()
                out["trainer_logs"]['balance_loss_mlp'] = balance_losses_mlp.sum(dim=-1).mean().detach().clone()
