...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    def __init__(self, dmodel: int, n_experts: int, expert_size: int, n_heads: int,
        ...
        assert self.perplexity_reg_mode in {"global", "time", "step", "layers_time", "standard", "tcmoe"}
        ...
        self.get_initializer()(self.values, std=self.size ** -0.5 * weight_scale)

        self.num_null_experts = self.n_heads                    # K null slots
        self.router_dim = 2 * self.n_experts + self.num_null_experts
        self.reward_factor = 1e-5

        self.expert_sel = torch.nn.Parameter(torch.empty(self.router_dim, self.k_vec_dim))
        self.sel_bias = torch.nn.Parameter(torch.zeros(self.router_dim)) if sel_bias else None
    ...
    def add_perplexity_reg_tcmoe(self, sel_aux: torch.Tensor, sel_index: torch.Tensor, bsz: int, seq_len: int):
        # only compute perplexity for non-null experts
        sel_aux_non_null, sel_aux_null = sel_aux[..., :2 * self.n_experts], sel_aux[..., 2 * self.n_experts:]
        
        # null mask
        null_mask = sel_index >= 2 * self.n_experts
        
        # Reward loss for null experts (matching original implementation)
        if self.reward_factor > 0 and self.training:
            null_prob = - self.reward_factor * sel_aux_null.sum() / (bsz * seq_len)    # for reward loss
            self.add_reg(lambda: null_prob, "moe")
        ...
        if self.perplexity_reg > 0:
            ...
            base_expert_idx = sel_index % self.n_experts
            non_null_mask = sel_index < 2 * self.n_experts
            
            # Create one-hot for non-null expert assignments
            aux_loss = torch.zeros(bsz, self.n_experts, device=sel_aux.device)
            valid_assignments = base_expert_idx[non_null_mask]
            batch_indices = torch.arange(bsz, device=sel_aux.device).unsqueeze(1).unsqueeze(2).expand(-1, seq_len, self.n_heads)[non_null_mask]
            ...
            p_avg = p_norm.mean(dim=(1, 2))  # [B, 2*n_experts]
            
            # For base experts, combine positive and negative probabilities
            p_base = p_avg[..., :self.n_experts] + p_avg[..., self.n_experts:2*self.n_experts]
            
            # Auxiliary loss
            load_balance_loss = (aux_loss * p_base).sum(dim=1).mean()
            
            self.add_reg(lambda: self.perplexity_reg * load_balance_loss, "moe")
    ...
    def sel_activation(self, sel: torch.Tensor, seq_len: int) -> Tuple[torch.Tensor, torch.Tensor]:
        reg_sel = sel.clone()  # Keep original for regularization
        
        # Apply biases before activation to match original implementation
        sel = sel.clone()  # Don't modify input tensor in-place
        # Negative experts (indices n_experts to 2*n_experts) get -1.0 bias
        sel[..., self.n_experts:2*self.n_experts] = sel[..., self.n_experts:2*self.n_experts] - 1.0
        # Null experts (indices 2*n_experts to end) get -10.0 bias  
        sel[..., 2*self.n_experts:] = sel[..., 2*self.n_experts:] - 10.0
        
        if self.selection_mode in {"sigmoid"}:
            sel = torch.sigmoid(sel)
        elif self.selection_mode in {"gate"}:
            # Use softmax for TCMoE (this is correct for the original implementation)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        weights_from_non_null_experts = sel_val * (sel_index < 2 * self.n_experts).float()
        ...
        weightsum_from_null_experts = sel_aux[..., 2 * self.n_experts:].sum(dim=-1, keepdim=True)
        ...
        gates = sel_aux / (weightsum + 1e-20)
        # Re-gather the normalized weights for selected experts
        sel_val = torch.gather(gates, -1, sel_index)

        # Map ternary indices to (base_expert, sign) & skip null experts
        base_idx = sel_index % self.n_experts                    # [B,T,K]
        sign = torch.ones_like(sel_val)                         # +1 default
        neg_mask = (sel_index >= self.n_experts) & (sel_index < 2 * self.n_experts)
        null_mask = sel_index >= 2 * self.n_experts
        ...
        if not self.training:
            ...
            for h1 in range(self.n_heads):
                for h2 in range(self.n_heads):
                    # Only consider valid expert indices (0 to n_experts-1) for coocurence
                    expert_idx1 = sel_index_flat[..., h1] % self.n_experts
                    expert_idx2 = sel_index_flat[..., h2] % self.n_experts

                    # Only count coocurence for valid expert pairs (both should be < n_experts)
                    valid_mask = (sel_index_flat[..., h1] < self.n_experts) & (sel_index_flat[..., h2] < self.n_experts)

                    if valid_mask.any():
                        ind_flat = expert_idx1[valid_mask] * self.n_experts + expert_idx2[valid_mask]
        ...
        if record_counts_now:
            reg_counts = F.one_hot(sel_index, self.router_dim).type_as(input)
            ...
            with torch.no_grad():
                ...
                if self.training and self.log_interval is not None:
                    ...
                    if self.iter % self.log_interval == 0:
                        ...
                        sel_oh = F.one_hot(sel_index, self.router_dim).sum(-2).bool()
        ...
        if reg_sel is not None:
            ...
            elif self.perplexity_reg_mode in {"tcmoe"}:
                self.add_perplexity_reg_tcmoe(sel_aux, sel_index, bsz, seq_len)
            elif self.perplexity_reg > 0 and self.training:
                self.sel_hist.append(reg_sel)

        # Prepare CVMM routing - keys are identical for +1/-1/0 variants → use base_idx
        idx_keys = cvmm_prepare_sel2(base_idx.int())
        scores = self.compute_scores(in2, idx_keys)

        # For TCMoE, we handle signs in the weights, not in separate value tensors
        # This is more efficient and matches the conceptual model better
        idx_vals = cvmm_prepare_sel2(base_idx.int()).clone()    
        
        # Apply sign-adjusted weights (including null expert masking)
        final_weights = sel_val * sign  # sign is 1.0 for positive, -1.0 for negative, 0.0 for null
        idx_vals.reduction_weight = final_weights   
        idx_vals.sel_index = idx_vals.out_index
        idx_vals.out_index = None
        
        # Aggregate value projections using base expert values with signed weights
        out = cvmm(scores, idx_vals, self.values)
