...
class ConstantExpert(torch.nn.Module):
    def __init__(self, out_embed_dim):
...
class ConstantExpert(torch.nn.Module):
    ...
    def forward(self, inputs):
        # inputs: (batch, d)
        weight = self.wg(inputs)            # (batch, 2)
        weight = self.softmax(weight)       # (batch, 2)

        # first term: weight[:,0] * inputs
        w0 = weight[:, 0].unsqueeze(1)     # (batch, 1)
        term1 = w0 * inputs                 # broadcasts to (batch, d)

        # second term: weight[:,1] * constant
        w1 = weight[:, 1].unsqueeze(1)     # (batch, 1)
        # self.constant is (d,), so broadcasting yields (batch, d)
        term2 = w1 * self.constant
...
class MoE(LoggingLayer, RegularizedLayer, OncePerIterLayer, torch.nn.Module):
    def __init__(self, dmodel: int, n_experts: int, expert_size: int, n_heads: int,
        ...
        self.n_experts_base = n_experts
        self.expert_size = expert_size
        self.size = self.n_experts_base * self.expert_size
        ...
        # initialize for the zero_expert, copy expert and constant expert
        self.zero_expert = ZeroExpert()
        self.copy_expert = CopyExpert()
        # initialize 6 ConstantExperts for the constant experts
        self.constant_experts = torch.nn.ModuleList([ConstantExpert(self.v_dim) for _ in range(6)])
        
        # update self.n_experts
        self.n_experts = self.n_experts_base + 1 + 1 + 6
