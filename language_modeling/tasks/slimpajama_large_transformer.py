...
@task()
class SlimpajamaLargeTransformer(LMEvalMixin, LMBase):
    ...
    def create_datasets(self):
        ...
        if self.helper.args.stop_after is not None:
            ...
            train_token_limit = self.helper.args.lm.unroll * self.helper.args.batch_size * (self.helper.args.stop_after + 100)
            # train_token_limit = 100000000000
        else:
        ...
        self.train_set = dataset.SlimPajamaLarge(self.helper.args.lm.unroll, split="train",
                                                 n_tokens=self.helper.args.sentencepiece.n_pieces, token_limit=train_token_limit)
     
        self.valid_sets.val = dataset.SlimPajamaLarge(self.helper.args.lm.unroll, split="validation",
                                                      n_tokens=self.helper.args.sentencepiece.n_pieces, token_limit=test_token_limit)
