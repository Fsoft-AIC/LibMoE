...
type_moelayer = "mm_projector"
for name_data in datas:
    for name_ml in model_list:
        data_selected_final = {} 
        path_root = f"/cm/archive/anonymous_new/revise_checkpoints/Xphi35-siglip224/SMOE/1M2/{name_ml}/analysts"
        ...
        def process(path_data):
            ...
            for sample in data['logs']:
                ...
                for id_layer, value in sample.items():
                    ...
                    if id_layer != type_moelayer: continue
                    new_sample[id_layer] = {}
            ...
            output_path = os.path.join(output_dir, name_data + "_processed_data_mm_projector.json")
            # Save new_data to a JSON file
            with open(output_path, 'w') as f:
                json.dump(new_data, f, indent=4)  # Using json for readability with indent
            with open(f"{path_root}/{name_data}_data_selected_final_mm_projector.json", 'w') as f:
        ...
        for name_model in data_selected_final.keys():
            ...
            for i in range(1, len(percent_data)):
                ...
                score_changes_final[name_model][i-1]['mm_projector'] = []
                changes_list = [
                    calculate_expert_changes(e1, e2)
                    for e1, e2 in zip(
                        data_selected_final[name_model][percent_data[i]][str('mm_projector')],
                        data_selected_final[name_model][percent_data[i - 1]][str('mm_projector')]
                    )
                ]
                score_changes = np.average(changes_list)
                score_changes_final[name_model][i-1]['mm_projector'].append(score_changes)

        with open(f"{path_root}/{name_data}_score_selected_final_mm_projector.json", 'w') as f:
