...
def calculate_expert_changes_order(epoch_1, epoch_2):
    
    score = []
    for token1, token2 in zip(epoch_1, epoch_2):
        changes = 0
      
        for e in range(len(token1)):

            changes += int(token1[e] == token2[e])
...
for name_data in datas:
    for name_ml in model_list:
        ...
        for name_model in data_selected_final.keys():
            ...
            for i in range(0, len(percent_data)-1):
                score_changes_final[name_model][int(percent_data[i])] = {}
                for id_layer in range(27):
                    
                    id_layer = str(id_layer)
                    score_changes_final[name_model][int(percent_data[i])][id_layer] = []
                    changes_list = [
                        calculate_expert_changes_order(e1, e2)
                        for e1, e2 in zip(
                            data_selected_final[name_model][percent_data[i]][str(id_layer)],
                            data_selected_final[name_model][percent_data[len(percent_data)-1]][str(id_layer)]
                        )
                    ]
                    score_changes = np.average(changes_list)
                    score_changes_final[name_model][int(percent_data[i])][id_layer].append(score_changes)

        with open(f"{path_root}/{name_data}_saturation_order_final.json", 'w') as f:
